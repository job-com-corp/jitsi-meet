...
function room_created(event)
    ...
    module:log("info", "Room created - %s", room.jid);
    
    local tenant = getTenantFromRoomName(room.jid);
    local roomname = jid.node(room.jid);    
    local URL_EVENT_ROOM_CREATED = api_protocol..'://'..tenant..'.'..api_domain..api_path..'/started/'..roomname;

    module:log("info", "POST URL - %s", URL_EVENT_ROOM_CREATED);
...
function room_destroyed(event)
    ...
    -- local tenant = getTenantFromRoomName(room.jid);
    -- local roomname = jid.node(room.jid);    
    -- local URL_EVENT_ROOM_DESTROYED = api_protocol..'://'..tenant..'.'..api_domain..api_path..'/stopped/'..roomname;
